...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: `Analyze web content specifically for Asqin BV market intelligence. Focus on:
          1. Company performance and growth indicators
          2. Investment signals and funding patterns
          3. Market positioning and competitive advantages
          4. Technology adoption and innovation trends
          ...
          {
            role: "user",
            content: `Analyze these URLs for Asqin BV market intelligence: ${urls.join(", ")}`,
          },
        ],
        response_format: { type: "json_object" },
      });
  ...
  // Add specialized STEPPS analysis endpoint
  app.post("/api/market-intelligence/stepps-analysis", requireAuth, async (req, res) => {
    if (!req.user) {
      return res.status(401).json({ message: "Unauthorized" });
    }
    try {
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: `Analyze content using the STEPPS framework for virality prediction:
          - Social Currency
          - Triggers
          - Emotion
          - Public
          ...
          Output JSON with scores and recommendations for each component.`,
          },
          {
            role: "user",
            content: `Analyze this ${contentType} content for virality potential:\n${content}`,
          },
        ],
        response_format: { type: "json_object" },
      });
      ...
      console.error("Error performing STEPPS analysis:", error);
      res.status(500).json({ message: "Failed to perform STEPPS analysis" });
    }
  });
